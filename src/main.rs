...
fn main() {
    ...
    let bounds = parse_pair(&args[2], 'x').expect("Error parsing the image dimensions");
    ...
    // Use 4 x 3 with 2 thread to understand
    let threads = 8;
...
fn pixel_to_point(
    bounds: (usize, usize),    // size of image in pixel
    pixel: (usize, usize),     // pixel location in (column, row)
    upper_left: Complex<f64>,  // top left point co ordinates
    lower_right: Complex<f64>, // bottom right point co ordinates
) -> Complex<f64> {
    let (width, height) = (
